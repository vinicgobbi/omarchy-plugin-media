...
- **Bar buttons and the widget MouseArea**: the widget-wide `MouseArea`
  ...
  it would swallow clicks meant for the previous / next buttons. The `Row`
  therefore has `z: 1`, and only the `BarButton`s have handlers; clicks on
  anything else in the row fall through to that `MouseArea`. Buttons go
